...
interface SimpleShippingTableProps {
...
const SimpleShippingTable: React.FC<SimpleShippingTableProps> = ({ onViewDetails }) => {
  ...
  const exportToExcel = () => {
    ...
    const excelData = SHIPPING_ROUTES.map(route => ({
      'Origin City': route.city_of_origin,
      'Origin Country': route.country_of_origin,
      'Destination City': route.city_of_destination,
      'Destination Country': route.country_of_destination,
      'Shipping Date': route.date_of_shipping,
      'Transit Time (Days)': route.total_shipping_time_days,
      'Price (USD)': route.price_of_shipping,
      'Currency': route.currency,
      'Container Type': route.container_type,
      'Carrier': route.carrier,
      'Provider': route.provider,
      'Distance (km)': route.distance,
      'Shipment ID': route.shipment_id,
      'CO2 Amount': route.co2_amount,
      'CO2 Price': route.co2_price,
      'Website Link': route.website_link,
      'Screenshot URL': route.screenshot_url,
      'Scraping Date': route.datetime_of_scraping,
      'Validity From': route.validity_from,
      'Validity To': route.validity_to
    }));
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Shipping Routes');

    // Generate Excel file and download
    const fileName = `shipping_routes_${new Date().toISOString().split('T')[0]}.xlsx`;
          ...
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              ...
              <p className="text-gray-600">
                Global shipping matrix with pricing and transit information
              </p>
            </div>
          </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{SHIPPING_ROUTES.length}</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {formatCurrency(SHIPPING_ROUTES.reduce((sum, r) => sum + r.price_of_shipping, 0) / SHIPPING_ROUTES.length, 'USD')}
                  </p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {calculateAverageTransitTime(SHIPPING_ROUTES)} days
                  </p>
                </div>
...
export default SimpleShippingTable; 